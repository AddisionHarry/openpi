...
def unparse_observation(message: Dict, device: str) -> Dict:
    ...
    return {
        "observation/image": chest_image,
        "observation/wrist_image": head_left_image,
        "observation/state": np.concatenate([
            # arm joints pos
            left_arm_joints_pos, right_arm_joints_pos,
            # hand joints pos
            left_hand_joints_pos, right_hand_joints_pos,
            # head & waist joints pos
            neck_joints_pos, waist_joints_pos,
            # arm joints vel
            left_arm_joints_vel, right_arm_joints_vel,
            # head & waist joints vel
            neck_joints_vel, waist_joints_vel,
            # eef
            left_tcp_pose_in_chest, right_tcp_pose_in_chest,
            # forces
            left_hand_force, right_hand_force,
            # tcp
            chest_in_eye
            ],axis=0),
        "prompt":"do something",
...
#  example = {
#         "observation/image": pil_to_uint8(frame["image"]),   
#         "observation/state": pad_to_dim(frame["state"], STATE_DIM),      
#         "prompt": frame.get("task", "")      
#     }
#     if "wrist_image" in frame:
#         example["observation/wrist_image"] = pil_to_uint8(frame["wrist_image"])
...
def main():
    ...
    # payload = torch.load(open(args.model_path, 'rb'), pickle_module=dill)
    # cfg = payload['cfg']
    # cls = hydra.utils.get_class(cfg._target_)
    # workspace: TrainDiffusionUnetImageIsaacLabWorkspace = cls(cfg, output_dir=args.output_dir)
    ...
    # pi0 fast
    cfg = config.get_config("pi0_fast_mine_low_mem_finetune")
    ckpt_dir = "openpi/checkpoints/pi0_fast_mine_low_mem_finetune/my_experiment/49999"
    # pi0
    # cfg = config.get_config("pi0_mine_low_mem_finetune")
    # ckpt_dir = "openpi/checkpoints/pi0_mine_low_mem_finetune/pi0_popcorn/49999"
    ...
    # policy.eval()
    # policy.num_inference_steps = 16  # Reduce diffusion steps for faster inference
    
    # Prepare observation deque
    obs_deque = deque([], maxlen=16)
                ...
                while True:
                    ...
                    try:
                        with torch.inference_mode():
                            ...
                            # nobs = stack_last_n_obs_dict(obs_deque, 16)
                            ...
                            response = json.dumps({
                                ...
                                # 'observation': {
                                #     "left_arm_joint_angles": message["left_arm_joint_angles"],
                                #     "right_arm_joint_angles": message["right_arm_joint_angles"],
                                #     "left_hand_joints": message["left_hand_joints"],
                                #     "right_hand_joints": message["right_hand_joints"],
                                #     "waist_angles": message["waist_angles"],
                                #     "neck_angles": message["neck_angles"]
                                # },
                                "timestamp": message["timestamp"]
                            })
