...
def load_zj_humanoid_train_configs_from_yaml(dir_path: str) -> List[TrainConfig]:
    ...
    for yaml_file in sorted(dir_path.glob("*.yaml")):
        ...
        name = yaml_file.stem
        assert name == cfg["name"], f"Config name should equal to the file name, file {yaml_file} got name {name}"
        model_cfg = cfg["model"]
        model = pi0_config.Pi0Config(
            dtype=model_cfg.get("dtype", "bfloat16"),
            paligemma_variant=model_cfg.get("paligemma_variant"),
            action_expert_variant=model_cfg.get("action_expert_variant"),
            action_dim=model_cfg.get("action_dim"),
            action_horizon=model_cfg.get("action_horizon"),
            max_token_len=model_cfg.get("max_token_len"),
            pi05=model_cfg.get("pi05", False),
            discrete_state_input=model_cfg.get("discrete_state_input"),
        )
        data_cfg = cfg["data"]
        data = LeRobotZJHumanoidDataConfig(
            repo_id=data_cfg["repo_id"],
            assets=AssetsConfig(
                assets_dir=data_cfg["assets"]["assets_dir"],
                asset_id=data_cfg["assets"]["asset_id"],
            ),
            base_config=DataConfig(
                prompt_from_task=data_cfg.get("base_config", {}).get("prompt_from_task", False),
            ),

            extra_delta_transform=data_cfg.get("extra_delta_transform", False),
            force_offline_dataset=data_cfg.get("force_offline_dataset", False),

            use_arms=data_cfg.get("use_arms", [False, True]),
            obs_use_waist_angles=data_cfg.get("obs_use_waist_angles", False),
            action_use_waist_angles=data_cfg.get("action_use_waist_angles", False),
            use_wrist_cameras=data_cfg.get("use_wrist_cameras", [False, True]),

            use_tcp_pose=data_cfg.get("use_tcp_pose", False),
            tcp_pose_in_wrist=data_cfg.get("tcp_pose_in_wrist", True),

            flip_wrist_images=data_cfg.get("flip_wrist_images", False),

            use_hand_align_state=data_cfg.get("use_hand_align_state", False),
            hand_align_state_chest_image_mask_prob=data_cfg.get(
                "hand_align_state_chest_image_mask_prob", 0.0
            ),
            hand_align_state_idx=data_cfg.get("hand_align_state_idx", 0),
        )
        weight_loader_cfg = cfg.get("weight_loader", {})
        if weight_loader_cfg.get("params_path"):
            weight_loader = weight_loaders.CheckpointWeightLoader(
                weight_loader_cfg["params_path"]
            )
        else:
        ...
        lr_schedule = _optimizer.CosineDecaySchedule(
            ...
            peak_lr=float(lr_cfg["peak_lr"]),
            decay_steps=lr_cfg["decay_steps"],
            decay_lr=float(lr_cfg["decay_lr"]),
        )
        train_config = TrainConfig(
            name=name,
            project_name=cfg.get("project_name", "openpi"),
            assets_base_dir=cfg.get("assets_base_dir", "./assets"),
            model=model,
            data=data,
            weight_loader=weight_loader,
            lr_schedule=lr_schedule,
            pytorch_weight_path=cfg.get("pytorch_weight_path"),
            pytorch_training_precision=cfg.get("pytorch_training_precision", "bfloat16"),
            num_train_steps=cfg.get("num_train_steps", 30000),
            log_interval=cfg.get("log_interval", 100),
            save_interval=cfg.get("save_interval", 1000),
            keep_period=cfg.get("keep_period"),
            batch_size=cfg.get("batch_size", 32),
            num_workers=cfg.get("num_workers", 2),
            ema_decay=cfg.get("ema_decay", 0.99),
            seed=cfg.get("seed", 42),
            wandb_enabled=cfg.get("wandb_enabled", True),
            force_offline_dataset=cfg.get("force_offline_dataset", False),
...
_CONFIGS = [
    ...
    #
    #
    *load_zj_humanoid_train_configs_from_yaml(pathlib.Path(__file__).resolve().parents[2] / "configs"),
    #
    # RoboArena configs.
    #
